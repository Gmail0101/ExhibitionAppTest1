...
<%
	...
	sql += " extitle=?,genre=?,author=?,rating=?,texts=?,placename=?,exprice=?,sdate=?,edate=?,docent=? where exidx=?";
	...
	pstmt.setString(11,exidx);
		...
		if(rs1 >0){
			out.println("<script>alert('수정완료'); location.replace('Exhibition.jsp')</script>");
		}
%>